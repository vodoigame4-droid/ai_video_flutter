...
class CreateVideoPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) =>
          sl<CreateVideoBloc>(param1: initialTab)
            ..add(CreateVideoEvent.init(initialTab)),
                  ...
                  ready: (readyState) async {
                    if (readyState.isGenerating) {
                      ...
                      if (readyState.selectedTab == 2) {
                        final cachedRemoteUrl =
                            readyState.uploadedSlotsPaths.isNotEmpty
                            ? readyState.uploadedSlotsPaths[0]
                            : null;
                        resolvedVideoUrl =
                            (cachedRemoteUrl != null &&
                                cachedRemoteUrl.isNotEmpty)
                            ? cachedRemoteUrl
                            : (readyState.slotsPaths.isNotEmpty
                                  ? (readyState.slotsPaths[0] ?? '')
                                  : '');
                      }

                      final hasInsufficient =
                          await CreditNavigationHelper.checkInsufficientCreditsAndNavigate(
                            context,
                      ...
                      String getPathOrUrl(int index) {
                        final cachedRemoteUrl =
                            readyState.uploadedSlotsPaths.length > index
                      ...
                      switch (readyState.selectedTab) {
                        ...
                        default:
                          ...
                          final resolvedPaths = List.generate(
                            readyState.slotsPaths.length,
                            getPathOrUrl,
                          );
                          imageUrl = resolvedPaths.firstWhere(
                            (p) => p.isNotEmpty,
                            orElse: () => '',
                          );
                      }
                      ...
                      GeneratingPage.push(
                        ...
                        isLongTime:
                            readyState.duration == '10s' ||
                            readyState.duration == '15s',
                              ...
                              children: [
                                ...
                                Padding(
                                  ...
                                  child: CreateVideoTabBarWidget(
                                    ...
                                    onHelpPressed: () {
                                      final guideImageUrls =
                                          sl<RemoteConfigService>()
                                              .getGuideImageUrls();
                                      final imageUrl =
                                          selectedTab >= 0 &&
                                              selectedTab <
                                                  guideImageUrls.length
